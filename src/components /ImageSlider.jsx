...
const ImageSlider = () => {
  ...
  // Hero content data - you can modify these
  const slides = [
    {
      id: 1,
    ...
    },
    {
      ...
      subtitle: "Pre-loved books in excellent condition, ready for their next adventure.",
    ...
    },
    {
      ...
      subtitle: "Connect with fellow book lovers and discover hidden literary gems.",
      image: "https://images.unsplash.com/photo-1524995997946-a1c2e315a42f?w=1200&h=600&fit=crop&crop=center",
      cta: "Learn More"
    }
  ];
        ...
        >
          ...
          <div className="relative h-full flex items-center justify-center text-center px-6">
            <div className="max-w-4xl mx-auto">
              <h1 className={`text-4xl md:text-6xl lg:text-7xl font-bold text-white mb-6 transform transition-all duration-1000 ${
                idx === current ? "translate-y-0 opacity-100" : "translate-y-10 opacity-0"
              }`}>
                {slide.title}
              </h1>
              <p className={`text-xl md:text-2xl text-gray-200 mb-8 max-w-2xl mx-auto transform transition-all duration-1000 delay-300 ${
                idx === current ? "translate-y-0 opacity-100" : "translate-y-10 opacity-0"
              }`}>
                {slide.subtitle}
              </p>
              <button className={`bg-gradient-to-r from-cyan-500 to-blue-600 text-white font-semibold px-8 py-4 rounded-full hover:from-cyan-600 hover:to-blue-700 transform hover:scale-105 transition-all duration-300 shadow-lg hover:shadow-xl transform transition-all duration-1000 delay-500 ${
                idx === current ? "translate-y-0 opacity-100" : "translate-y-10 opacity-0"
              }`}>
                {slide.cta}
              </button>
